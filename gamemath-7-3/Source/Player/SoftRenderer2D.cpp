...
void SoftRenderer::Render2D()
{
	...
	Vector2 prjectedPoint = lineStart + projV;
	float distance = (prjectedPoint - point).Size();

	// 투영된 점 그리기
	for (auto const& v : circle)
	{
		r.DrawPoint(v + prjectedPoint, LinearColor::Magenta);
	}

	// 투영 라인 그리기
	r.DrawLine(prjectedPoint, point, LinearColor::Gray);

	// 관련 데이터 화면 출력
	r.PushStatisticText(std::string("Point : ") + point.ToString());
	r.PushStatisticText(std::string("Projected Point : ") + prjectedPoint.ToString());
